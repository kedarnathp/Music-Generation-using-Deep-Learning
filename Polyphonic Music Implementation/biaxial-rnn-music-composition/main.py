...
def gen_adaptive(m,pcs,times,keep_thoughts=False,name="final"):
	...
	noteStateMatrixToMidi(numpy.array(all_outputs),'output_piano_200/'+name)
	if keep_thoughts:
		pickle.dump(all_thoughts, open('output_piano_200/'+name+'.p','wb'))
...
if __name__ == '__main__':
	...
	pcs = multi_training.loadPieces("piano_music_data")

	m = model.Model([200,200],[100,50], dropout=0.5)

#	multi_training.trainPiece(m, pcs, 20000)

#	pickle.dump( m.learned_config, open( "output_piano_3/final_learned_config.p", "wb" ) )

	m.learned_config = pickle.load(open( "output_piano_200/final_learned_config.p", "rb" ) )

	gen_adaptive(m,pcs,5,name="composition_1")
